---
title: "Computing uncertainty and sensitivity"
output:
  html_document:
    df_print: paged
...
vignette: "%\\VignetteIndexEntry{Example uncertainty and sensitivity} %\\VignetteEncoding{UTF-8}
...
To show how uncertainty spread through the model and over time we have plotted a 95% confidence band around the expected total system carbon projection. Uncertainty grows over time, though it is reset to almost zero every time there is a canopy-burn episode every 500 years.
...
To determine model sensitivity to each texture parameters we calculate the correlation coefficient between each parameter and the output.

```{r calc_corr, fig.width = 6, fig.height = 5, fig.align = 'center', fig.show = 'hold'}
nx <- length(x)
cc <- sapply(2:nx, function(i) sapply(1:3, function(j) cor(texture[j,], z[i,])))
plot(2:nx, cc[1, ], ylim = c(-1, 1), xlab = "Time (years)", ylab = "Correlation coeff.",
...
Notice how sensitivity of model output to the values of the silt parameter is very high at the beginning, but dramatically drops to values close to zero after a few years. However, sensitivity to clay and sand values are high and of opposite sign.

<br>

To determine the relative contribution of each texture parameter to model output we can compute the $R^2$ coefficient for each texture parameter and then normalized it such that the sum of the $R^2$ from each parameter is equal to one. 

```{r calc_sens, fig.height = 6, fig.width = 5, fig.align = 'center', fig.show = 'hold'}
nx <- length(x)
...
Clearly, clay content is the most important texture parameter, followed at some distance by sand. The contribution of silt values, on the other hand, is almost negligible.
...
$$\tilde{\alpha_i}=\frac{\alpha_i}{\alpha_0}$$ We can use those two expressions to calculate the $\alpha_j$ values when we are given four parameters for the $E[x_i]$ or $E[V_i]$. The Rcentury package provides with a function, called **dirichlet_params.R**, that can calculate the $\alpha_j$ values when all the means $E[x_i]\text{ for all } i \in \{1, \dots, K\}$ and one single variance value $Var[x_j]$ are known.
...
Notice that not all combinations of $E[x_i]\text{ for all } i \in \{1, \dots, K\}$ and $Var[x_j]$ are valid. Introducing $Var[x_2]=.28$ will rise a warning message and negative values for all the $\alpha_i$.
